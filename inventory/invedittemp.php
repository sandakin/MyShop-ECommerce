...
<head>
 ...
 <script type="text/javascript">
  function popitup(url) {
       newwindow=window.open(url,'windowName1','height=600,width=600');
...
if (!mysqli_query($bd, $sql2))
 ...
 if (!mysqli_query($bd, $sql3))
  ...
  die('Error: ' . mysqli_error($bd));
   ...
   $result2 = mysqli_query($bd,"SELECT * FROM `product_inv` WHERE `inv_id` = '$_GET[invid]' ");
    while($r2 = mysqli_fetch_array($result2)) {
   
   $sql4 = "INSERT INTO `shop`.`invtemp2` (`p_id`, `u_price`, `quantity`) VALUES ('$r2[p_id]', '$r2[p_uprice]', '$r2[p_pquantity]')";
    if (!mysqli_query($bd, $sql4))
  {
  die('Error: ' . mysqli_error($bd));
  }
}
...
<form id="form1" name="form1" method="post" action="invedittemp.php?filter=1" enctype="multipart/form-data">
</br>
...
while ($temp2r = mysqli_fetch_array($sqltemp2)) {
  ...
  <tr>
    <td> <?php 
      $pnamesql=mysqli_query($bd,"SELECT * FROM `product` WHERE `p_ID` = '$temp2r[p_id]'");
        $pnamere = mysqli_fetch_assoc($pnamesql);
    echo $pnamere['p_name']; ?> </td>
  	<td> <?php echo $temp2r['u_price']; ?></td>
  	<td> <?php echo $temp2r['quantity']; ?></td>
    <td><a onclick="popitup('remove1.php?pid=<?php echo $pnamere['p_ID']; ?>')" type="button" class="btn glyphicon glyphicon-remove btn-danger"></a></td>
  </tr>


<?php } ?>
</table>
 <a href="updateinventory.php"> <input class="btn btn-primary" value="Confirm"></a>
</div>
