...
if(isset($_GET['edit'])){
	echo "not here";
	$id=mysql_real_escape_string($_GET['paym_ID']); 

// $sql = "UPDATE `shop`.`category` SET  `c_name` = '$_POST[c_name]', `c_desc` = '$_POST[c_desc]' WHERE `category`.`c_ID` = $_GET[c_ID];";
$sql = "UPDATE `shop`.`pay-methods` SET  `paym_name` = '$_POST[pay_name]', `paym_desc` = '$_POST[pay_desc]' WHERE `pay-methods`.`paym_ID` = $_GET[paym_ID];";
