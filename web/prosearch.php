...
$key=1;
echo"<div class='well'>  ";
echo "search results for :".$srch."<br>";
echo" </div>";
...
$key=2;
echo"<div class='well'>  ";
echo "search results for category :".$catid."<br>";
echo" </div>";
}
...
if ($key==1) {
  ...
  $sql2 = "SELECT * ".
       "FROM product WHERE `p_name` like '%$srch%'".
      "LIMIT $offset, $rec_limit   ";
} else if ($key==2){
 $sql2 = "SELECT * ".
       "FROM product WHERE `cat_ID` = '$catid'".
...
if (mysqli_num_rows($retval)==0) {
 echo "<div class='alert alert-warning' role='alert'>no product found,try a different keyword or category.</div>";
}else{
...
// else if( $left_rec < 0 )
{
   ...
   echo "<li class='previous'><a href='prosearch.php?page=$last&catid=$catid'><span aria-hidden='true'> </span> <span class='glyphicon  glyphicon-arrow-left' aria-hidden='true'></span>Previous</a></li>";
...
echo "<li class='next'><a href='prosearch.php?page=$page&catid=$catid'><span aria-hidden='true'> </span> Next<span class='glyphicon  glyphicon-arrow-right' aria-hidden='true'></span></a></li>";
...
}else if (isset($_POST['srch']) || isset($_GET['srch']))
{
  echo "<nav>  <ul class='pager'>";
...
// else if( $left_rec < 0 )
{
   ...
   // <li class="next"><a href="#">Newer <span aria-hidden="true">&rarr;</span></a></li>
          // echo "<li><a href='\"probrowse.php?page=$last\"'>Previous </a> </li>";
   echo "<li class='previous'><a href='prosearch.php?page=$last&srch=$srch'><span aria-hidden='true'> </span> <span class='glyphicon  glyphicon-arrow-left' aria-hidden='true'></span>Previous</a></li>";
   echo "<li class='next disabled'><a href=''>Next <span class='glyphicon  glyphicon-arrow-right' aria-hidden='true'> </a></li>";
...
echo "<li class='next'><a href='prosearch.php?page=$page&srch=$srch'><span aria-hidden='true'> </span> Next<span class='glyphicon  glyphicon-arrow-right' aria-hidden='true'></span></a></li>";
...
// {
//    $last = $page - 2;
//    echo "<a href=\"prosearch.php?page=$last&catid=$catid\">Last 10 Records</a> |";
//    echo "<a href=\"prosearch.php?page=$page&catid=$catid\">Next 10 Records</a>";
